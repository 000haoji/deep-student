    ...
    F: Fn(usize, usize),
{
    info!(
        ...
        backup_dir, app_data_dir, assets.len()
    );
    ...
    F: Fn(usize, usize),
{
    ...
    for entry in fs::read_dir(assets_dir)? {
        ...
        let dest_base = app_data_dir.join(&asset_type_name);
        let count = copy_dir_recursive_with_progress(&path, &dest_base, &mut restored_count, total, &on_progress)?;
    ...
    F: Fn(usize, usize),
{
    ...
    for entry in fs::read_dir(src)? {
        ...
        if src_path.is_dir() {
            count += copy_dir_recursive_with_progress(&src_path, &dest_path, restored_count, total, on_progress)?;
        } else {
            if let Some(parent) = dest_path.parent() {
                fs::create_dir_all(parent)?;
            }
