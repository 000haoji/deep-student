...
impl BackupManager {
    ...
    pub(crate) fn check_manifest_compatibility(&self, manifest: &BackupManifest) -> Result<(), BackupError> {
        // 1. 检查 manifest 格式版本
    ...
    pub(crate) fn rollback_from_pre_restore(&self, pre_restore_dir: &Path) -> Result<(), BackupError> {
    ...
    /// 创建预恢复备份
    pub(crate) fn create_pre_restore_backup(&self, pre_restore_dir: &Path) -> Result<(), BackupError> {
        // 清理旧的预恢复备份
        if pre_restore_dir.exists() {
