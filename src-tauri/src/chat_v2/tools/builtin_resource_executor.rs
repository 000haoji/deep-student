...
impl BuiltinResourceExecutor {
    ...
    async fn execute_mindmap_create(
        ...
        if let Some(ref vid) = version_id {
            ...
            result["hint"] = json!("请在回复中使用上方 citation 字段的引用文本，让用户可以点击查看导图。");
        }
            ...
            Ok(Some(new_content)) => {
                match VfsMindMapRepo::create_version(
                    ...
                    Ok(version) => {
                        ...
                        result["citation"] = json!(format!("[思维导图:{}:{}]", version.version_id, mindmap.title));
            ...
            match VfsMindMapRepo::get_mindmap_content(vfs_db, mindmap_id) {
                Ok(Some(new_content)) => {
                    ...
                    match VfsMindMapRepo::create_version(
                        ...
                        Ok(version) => {
                            result["versionId"] = json!(version.version_id);
                            result["citation"] = json!(format!("[思维导图:{}:{}]", version.version_id, title));
...
mod tests {
    ...
    fn test_strip_namespace() {
        assert_eq!(
            BuiltinResourceExecutor::strip_namespace("builtin-resource_list"),
            "resource_list"
        );
        assert_eq!(
            BuiltinResourceExecutor::strip_namespace("builtin-resource_read"),
            "resource_read"
        );
        // 无前缀时返回原字符串
        assert_eq!(
            BuiltinResourceExecutor::strip_namespace("resource_list"),
            "resource_list"
        );
    }
