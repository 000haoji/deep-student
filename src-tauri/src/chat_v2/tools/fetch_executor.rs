...
/// 移除 footer 页脚
static RE_FOOTER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<footer[^>]*>.*?</footer>").unwrap());
/// 移除 header 页眉
static RE_HEADER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<header[^>]*>.*?</header>").unwrap());
/// 移除 aside 侧边栏
static RE_ASIDE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<aside[^>]*>.*?</aside>").unwrap());
/// 移除 noscript 标签
static RE_NOSCRIPT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<noscript[^>]*>.*?</noscript>").unwrap());
/// 移除 iframe 标签
static RE_IFRAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<iframe[^>]*>.*?</iframe>").unwrap());
/// 移除 svg 标签
static RE_SVG: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<svg[^>]*>.*?</svg>").unwrap());
/// 移除 form 表单
static RE_FORM: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<form[^>]*>.*?</form>").unwrap());
/// 移除 dialog 对话框
static RE_DIALOG: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<dialog[^>]*>.*?</dialog>").unwrap());
/// 移除 template 模板
static RE_TEMPLATE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<template[^>]*>.*?</template>").unwrap());
/// 移除 object 嵌入对象
static RE_OBJECT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<object[^>]*>.*?</object>").unwrap());
/// 移除 embed 嵌入元素
static RE_EMBED: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<embed[^>]*(?:/>|>.*?</embed>)").unwrap());
/// 移除 applet 遗留元素
static RE_APPLET: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<applet[^>]*>.*?</applet>").unwrap());
/// 提取 <article> 内容
static RE_ARTICLE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<article[^>]*>(.*?)</article>").unwrap());
/// 提取 <main> 内容
static RE_MAIN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?is)<main[^>]*>(.*?)</main>").unwrap());
